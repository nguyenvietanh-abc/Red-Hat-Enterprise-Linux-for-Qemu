...
10. [Kết quả tổng hợp](#10-kết-quả-tổng-hợp)
11. [Kiến thức cốt lõi](#kiến-thức-cốt-lõi)
...
### 1. Quản trị hệ thống mức thấp
- **systemctl** — enable, start, kiểm tra status các dịch vụ (`sshd`, `nfs-server`, ...)
- **firewall-cmd** — mở port/service cho SSH, NFS, RPC
- **nmcli** — quản lý mạng (basic)

### 2. Quản lý disk & tối ưu tài nguyên
- **LVM full flow:** `pvcreate` → `vgextend` → `lvextend` → `xfs_growfs`
- Resize disk ảo trực tiếp trong VirtualBox (`VBoxManage modifymedium`)
- Theo dõi realtime: `free -h`, `df -h`, `vgs`, `lvs`

### 3. Mạng & chia sẻ file cho nhúng
- SSH server setup + mở firewall
- NFS server setup: `/etc/exports`, `exportfs -ra`, cấu hình firewall services
- Test mount NFS từ client → cách debug cho QEMU / board nhúng

### 4. An toàn & quy trình làm việc
- Không dùng `root` lâu dài → tạo user riêng + thêm vào `wheel` group
- Test sau mỗi bước lớn: `systemctl status`, `exportfs -v`, mount & `touch` file kiểm tra

### 5. Mindset phát triển nhúng
- Đảm bảo đủ disk / RAM **trước** khi build Yocto
- NFS là 1 cách chia sẻ rootfs / debug  — không cần flash hay SCP từng lần
- Xác nhận từng bước bằng lệnh kiểm tra (`status`, `df`, `ls`, `exportfs -v`) thay vì giả định thành công
